...
const userSchema = new mongoose.Schema({
    ...
    status: { type: String, enum: ['Disponible', 'Occupé', 'Absent'], default: 'Disponible' }
}, { timestamps: true });
...
async function addUser(userData) {
    try {
        ...
        if (existingUser) {
            console.log(`ℹ️ L'utilisateur avec l'email ${userData.email} existe déjà.`);
            return;
        }
...
async function main() {
    try {
        ...
        mongoose.connection.close(() => console.log("🔌 Connexion MongoDB fermée."));
    }
}
